...
$( document ).ready(function() {
    ...
    tabla = $("#t_categorias").DataTable({
        ...
        "order": [],
        "ajax": {
            "url":  default_server+"/json/comunicados/categorias",
            "type": "GET",
            "data": function ( d ) { },
        ...
        "columns": [            
            ...
            { "data": "comunicados_count", "searchable": false, className: "w-1 text-center",
                render: function ( data, type, full ) {                      
                    return data;
                }        
            },             
...
function guardar() 
{
    if(validar('#form_editar'))
    {
        ...
        else
        {
            $.ajax({
                type: "PUT",
                url: default_server+"/json/comunicados/categorias/"+editItem,
                data: {                   
                    nombre: $("#nombre").val(),
                    descripcion: $("#descripcion").val(),                    
                    estado: ($("#estado").is(':checked') ? 1 : 0 )
...
function eliminar(iditem) {
    if(confirm('Esta seguro que desea eliminar?'))
    {
        $.ajax({
            type: "DELETE",
            url: default_server+"/json/comunicados/categorias/"+iditem,        
            success: function(result){  
                alerta(result.message, true); 
                tabla.ajax.reload();
            },
