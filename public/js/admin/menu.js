...
$( document ).ready(function() {
    ...
    tabla = $("#t_categorias").DataTable({
        ...
        "columns": [            
            ...
            { "data": "menu_count", "searchable": false, className: "w-1 text-center",
                render: function ( data, type, full ) {                      
                    return data;
                }        
            },    
            { "data": "flag", "searchable": false, "visible": false},             
...
function modificar(iditem) 
{
    ...
    if(objitem != null)
    {
        ...
        $("#editar").modal("show");
        if(objitem.menu_id!=null){
            $('[id="tipo"]').val('1');
            $('[id="menu_id"]').val(objitem.menu_id);
...
function guardar() 
{
    if(validar('#form_editar'))
    {
        ...
        else
        {
            $.ajax({
                type: "PUT",
                url: default_server+"/json/paginas/menus/"+editItem,
                data: {                   
                    nombre: $("#nombre").val(),                    
                    fuente: $("#fuente").val(),
                    tipo: $("#tipo").val(),
...
function eliminar(iditem) {
    if(confirm('Esta seguro que desea eliminar?'))
    {
        $.ajax({
            type: "DELETE",
            url: default_server+"/json/paginas/menus/"+iditem,        
            success: function(result){  
                alerta(result.message, true); 
                tabla.ajax.reload();
            },
...
function tipo_menu(item) {
    if($(item).val() != 0){
        $("#div_menu").removeClass("oculto");
        if(editItem != 0){
            var data = tabla.rows().data();
            var objitem = elementId(editItem, data);
