...
function guardar() {
    if(confirm("Esta seguro que desea guardar?")){
        if(validar("#form_guardar")){
            ...
            formulario.append('carrera_id', $("#carrera_id").val()); 
            formulario.append('nombre', $("#nombre").val());
            formulario.append('cv', $("#cv").val());
            ...
            $.ajax({
                type: "POST",
                url: default_server+(elDocente != 0 ? '/json/docentes/'+elDocente : '/json/docentes'),
                processData: false,
                contentType: false,
                data: formulario,
                success: function(result){  
                    alerta(result.message, true); 
                    location.href = default_server+'/admin/docentes';
                },
