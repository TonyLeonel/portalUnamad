...
$( document ).ready(function() {
    ...
    tabla = $("#t_comunicados").DataTable({
        ...
        "order": [],
        "ajax": {
            "url":  default_server+"/json/comunicados",
            "type": "GET",
            "data": function ( d ) {
                d.categoria_id = $("#categoria_select").val(); 
             },
        ...
        "columns": [            
            ...
            { "data": "fecha_fin", "searchable": false, className: "w-1",
                render: function ( data, type, full ) {                      
                    return '<span class="'+(es_activo(data) ? 'text-success' : 'text-muted')+'">'+dis_fecha(data)+'</span>';
                }        
            }, 
            ...
            { 
                ...
                "searchable": false, "orderable": false, className: "w-1",
                    ...
                    var res = '<div class="btn-list flex-nowrap">'+
                                '<a class="btn btn-white btn-icon" href="'+default_server+'/admin/comunicados/modificar/'+full.id+'" title="MODIFICAR">'+
...
function eliminar(iditem) {
    if(confirm('Esta seguro que desea eliminar?'))
    {
        $.ajax({
            type: "DELETE",
            url: default_server+"/json/comunicados/"+iditem,        
