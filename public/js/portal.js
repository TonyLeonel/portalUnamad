...
initIsotope();
    ...
    $('.close-reg , .close-modal , .reg-overlay').on("click", function () {
        modal.hide();
    });
    function csselem() {
...
function initparallax() {
    ...
    if (trueMobile) {
		$(".background-youtube-wrapper").remove();
...
function compartir_copiar() {
    var ruta = $("#input_compartir").val();
    navigator.clipboard.writeText(ruta);
}

function compartir_facebook() {
    var ruta = $("#input_compartir").val();
    window.open("https://www.facebook.com/sharer/sharer.php?kid_directed_site=0&sdk=joey&u="+ruta+"&display=popup&ref=plugin&src=share_button", '_blank').focus();
}

function compartir_whatsapp() {
    var ruta = $("#input_compartir").val();
    window.open("https://api.whatsapp.com/send?text=Les comparto el siguiente enlace "+ruta, '_blank').focus();
}

function compartir_twitter() {
    var ruta = $("#input_compartir").val();
    window.open("http://twitter.com/share?text=Les comparto el siguiente enlace&url="+ruta+"&hashtags=post", '_blank').focus();
}